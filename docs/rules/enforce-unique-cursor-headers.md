...
- `allowSplitHeaders`: When false, metadata spread across multiple adjacent comment blocks is reported as a split header fragment, and the fixer does not merge or delete those fragments — combine them into a single block by hand. (Adjacent blocks that *each* carry every required tag are a different case: they report as duplicate headers and the fixer deletes the extras, under either setting of this option.)
