...
The walk is a deliberate **over-approximation**, so it can decline where TypeScript would have coped. A value binding is one node carrying its whole initializer's dependencies, so reading one member of an object literal inherits its unannotated siblings' — TypeScript resolves those members independently. A read in statement position counts the same as one on the return path. Both cost a missed report rather than a broken fix, which is the direction to err in. Some links are missed instead: a binding introduced by a destructuring pattern, a parameter default, and a member declared under a computed key are not nodes at all.
