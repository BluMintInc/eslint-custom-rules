...
- **A binding that cannot be unbound cleanly cancels that annotation's fix.** The report then carries no fixer, and you resolve it by hand — by dropping the declaration or by using it. This covers a locally declared `type` alias or `interface`, a type parameter, an import behind a `// eslint-disable-next-line` or `@ts-expect-error` directive, a comment sitting among the specifiers, and a name that another binding shadows. Such an annotation drops out of the batch without taking its siblings' fixes with it; only when the batch as a whole would orphan something unrewritable — two annotations sharing a local declaration, say — is the fix declined for all of them.
