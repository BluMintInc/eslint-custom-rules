...
- A type the checker cannot resolve is compared by **how it is written** instead. Without `parserOptions.project` there is no lib to resolve globals against, so `string[]` and `number[]` both collapse to the same anonymous `{}` and every generic reference becomes the error type — assignable in both directions and indistinguishable from one another. Reading that collapse as a match reported unrelated pairs and removed the annotation, silently changing the declared type. Identical spellings in one scope still denote one type, so a genuinely redundant pair is reported either way; a pair that merely looks alike to an unresolved checker is not.
