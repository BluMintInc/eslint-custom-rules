...
- Skips object literals that are the direct value of a JSX attribute named `sx` or `style` (e.g. `<Stack sx={{ ... }} />`, `<div style={{ ... }} />`). These style descriptors are consumed by the library without referential equality checks, so inlining them does not break memoization or trigger extra renders.
