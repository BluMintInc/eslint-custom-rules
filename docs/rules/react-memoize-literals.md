...
- Skips literals that resolve to a JSX attribute named `sx` or `style` (e.g. `<Stack sx={{ ... }} />`, `<div style={{ ... }} />`). The exemption follows the value through conditional branches (`sx={active ? { ... } : { ... }}`), logical fallbacks (`sx={active && { ... }}`), array entries (`sx={[{ ... }, { ... }]}`, supported by MUI), and `as const`/parenthesized wrappers. These style descriptors are consumed by the library without referential equality checks, so inlining them does not break memoization or trigger extra renders. Literals that are instead passed through a function call (`sx={makeSx({ ... })}`) or attached to a non-style prop remain reported, since those references can be observed or stored.
