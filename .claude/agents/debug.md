---
name: debug
description: "Use for complex debugging requiring hypothesis generation and root-cause analysis."
...
- **Context first with System Guides.** When the bug touches a core system documented in `.claude/skills/`, study the relevant SKILL.md guides to understand contracts, invariants, and pitfalls. Cite guides in your plan by path.
...
- Attach key artifacts (console excerpts, error messages) to the plan.
...
- Identify **false positives** (over-mocking, missing awaits, brittle timers, incorrect expectations). Note any tests that pass but mask the bug.
...
### 2.6) **Consult relevant System Guides (mandatory when applicable)**

- Identify the system(s) implicated by reading imports, directories, and call-sites. Then study the corresponding SKILL.md guides in `.claude/skills/` to gain full context and known invariants.
- Guides directory: `.claude/skills/`.
...
4. **Related Guides Consulted**
   List the SKILL.md guides consulted (by path in `.claude/skills/`) and summarize 1–3 critical invariants or contracts from each that informed your investigation.
