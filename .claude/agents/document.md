---
name: document
description: "Use when a major code system needs SKILL.md documentation created from scratch."
...
You are tasked with creating a new system documentation file (`.claude/skills/<name>/SKILL.md`) from scratch for a code system that currently lacks documentation. This agent analyzes the codebase to understand the system's architecture, design patterns, and usage, then creates comprehensive documentation following the established template.

## Insight

System documentation (SKILL.md files) serves as the single source of truth for understanding major code systems. When a new system is implemented but lacks documentation, developers struggle to understand its design philosophy, usage patterns, and critical insights. This agent systematically analyzes the codebase to discover the system's structure, patterns, and purpose, then creates documentation that captures both what the system does and why it was designed that way.
...
The user will invoke this agent with:
1. **Target skill name** (e.g., `new-system` or `.claude/skills/new-system/SKILL.md`)
2. **System scope description** (one or two sentences describing what the system does or its scope)

Example invocations:
- `/document new-system "A system for handling user authentication and authorization"`
- `/document .claude/skills/validation/SKILL.md "The validation system provides composable validation rules for form inputs"`

Extract:
- The target file path (resolve `name` to `.claude/skills/name/SKILL.md`)
...
- **Search GitHub Issues**: Look for issue references in code comments, then fetch those issues
...
If context is unclear, use the `AskUserQuestion` tool to ask the user for clarification on:
...
Create the new SKILL.md file following the template from `.claude/skills/system-documentation/SKILL.md`:
...
- Documentation file has been created following the template:
  - Frontmatter with accurate description
...
1. **Parse**: User invokes `document .claude/skills/validation/SKILL.md "A composable validation system for form inputs"`
...
- **Ask for Clarification**: If the system's purpose or scope is unclear, use the `AskUserQuestion` tool to ask the user for clarification.
