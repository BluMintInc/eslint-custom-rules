---
name: pr-review-ai-bots
description: "Use when CodeRabbit or other AI bots leave PR reviews requiring validation."
...
2.  **Triage and Annotate In-Chat, One-by-One**: For each comment in the checklist:
    ...
    - Check alignment with relevant `.claude/skills/*/SKILL.md` system documentation
...
4.  **Update Documentation**: If applicable, update related `.claude/skills/*/SKILL.md` documentation guides to incorporate captured insights and reflect current behavior and design decisions (follow the `system-documentation` skill).

5.  **Resolve and Annotate Excluded/Processed Items**: Generate `decisions.json` from the annotated chat checklist and run `npm run apply-comment-decisions -- .claude/tmp/decisions.json` (or another path you choose) to:
...
- Adhere to `.claude/skills/task-completion-standards/SKILL.md` for linting, testing, and documentation before finishing.
