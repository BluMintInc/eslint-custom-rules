...
description: "Use after implementing changes that modify a documented system."
...
You are tasked with updating an existing system documentation file (`.claude/skills/<name>/SKILL.md`) to reflect changes made in a Pull Request. This agent ensures that system documentation stays current with code changes and captures important design insights that may exist outside the code itself.

## Insight

System documentation (SKILL.md files) serves as the single source of truth for understanding major code systems. When systems evolve through PRs, the documentation must evolve too. PR descriptions, comments, and associated issues often contain valuable context about design decisions, motivations, and insights that aren't obvious from code diffs alone. This agent systematically gathers that context and updates the documentation accordingly.
...
- Only pause if blocked by missing information (use the `AskUserQuestion` tool to ask the user for what you need).
...
The user will invoke this agent with a target documentation file, for example:
- `update-documentation .claude/skills/geolocation/SKILL.md`
- `update-documentation geolocation`

Extract the target file path. If the user provided just a name like `geolocation`, resolve it to the full path `.claude/skills/geolocation/SKILL.md`.

**Optional PR Number**: If the user provided a PR number (e.g., `update-documentation geolocation PR#37046`), use that. Otherwise, proceed to step 2 to determine the PR from the current branch.

### 2. Gather PR Context

First, determine the PR number. If the user invoked this agent with a PR number argument, use that. Otherwise, get the current branch and fetch the associated PR number:
...
- **PR Diff**: `gh pr diff <PR_NUMBER> | cat` to get the files changed
- **PR Description**: `gh pr view <PR_NUMBER> --json body --jq .body | cat`
- **PR Title**: `gh pr view <PR_NUMBER> --json title --jq .title | cat`
- **CodeRabbitAI Walkthrough**: `gh pr view <PR_NUMBER> --json comments --jq '.comments[] | select(.author.login == "coderabbitai") | .body' | cat | head -n 200`
- **All PR Comments**: `gh pr view <PR_NUMBER> --json comments --jq '.comments[] | {author: .author.login, body: .body}' | cat`
...
- **PR Merge Status**: `gh pr view <PR_NUMBER> --json state,merged,baseRefName --jq '{state, merged, baseRefName}' | cat`
...
- **Map File Changes to System**: Identify which changed files belong to the system being documented
- **Identify System-Specific Changes**: Filter the PR diff to focus on files that match the system's scope
...
Update the target SKILL.md file following these guidelines:
...
- **Structure**: Follows the template structure from `.claude/skills/system-documentation/SKILL.md`
...
Here's an example of updating `geolocation` skill after PR #37046:

1. **Parse**: User invokes `update-documentation .claude/skills/geolocation/SKILL.md`
2. **Gather PR Context**: Fetch PR #37046 diff, description, comments, and associated issues
3. **Read Current Docs**: Read `.claude/skills/geolocation/SKILL.md` to understand current structure
...
- **Focus on the System**: While PRs may change many files, focus documentation updates on changes relevant to the target system
