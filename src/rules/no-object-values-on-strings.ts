...
> = createRule({
  create(context) {
     ...
     */
    function hasFunctionStringParameter(
      node:
    ...
    return {
      ...
      'FunctionDeclaration, FunctionExpression, ArrowFunctionExpression'(
        ...
        if (node.body && node.body.type === AST_NODE_TYPES.BlockStatement) {
          sourceCode.ast.body.forEach(function findObjectValuesCalls(
            statement,
          ) {
            if (
  ...
  meta: {
    ...
    docs: {
      description:
        'Disallow Object.values() on strings as it treats strings as arrays of characters, which is likely unintended behavior.',
      recommended: 'error',
    },
    schema: [],
    messages: {
