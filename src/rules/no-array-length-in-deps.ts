...
function isHookCall(node: TSESTree.CallExpression): boolean {
	...
	return callee.type === AST_NODE_TYPES.Identifier && HOOK_NAMES.has(callee.name);
}

function isLengthMemberExpression(node: TSESTree.Expression): node is TSESTree.MemberExpression {
...
function getLengthMember(node: TSESTree.Expression): TSESTree.MemberExpression | null {
	if (node.type === AST_NODE_TYPES.MemberExpression && isLengthMemberExpression(node)) {
		return node;
	}
	if (node.type === AST_NODE_TYPES.ChainExpression) {
...
function getBaseExpression(member: TSESTree.MemberExpression): TSESTree.Expression {
...
function collectAllTakenNames(context: TSESLint.RuleContext<MessageIds, []>): Set<string> {
...
function isUseMemoImported(sourceCode: TSESLint.SourceCode): boolean {
	...
	for (const node of program.body) {
		if (node.type === AST_NODE_TYPES.ImportDeclaration && node.source.value === 'react') {
	...
	create(context) {
		...
		return {
			CallExpression(node) {
				...
				// Collect .length deps
				const lengthDeps: { element: TSESTree.Expression; member: TSESTree.MemberExpression }[] = [];
				...
				context.report({
					...
					fix(fixer) {
						...
						if (needUseMemo) importText += `${indent}import { useMemo } from 'react';\n`;
						if (needStableHash)
						...
						if (importDecls.length === 0) {
							...
							if (declText || importText) {
								...
								if (needUseMemo) importTextNoIndent += `import { useMemo } from 'react';\n`;
								if (needStableHash)
