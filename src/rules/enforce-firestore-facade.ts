  ...
  create(context) {
    ...
    const recordFirestoreVariable = (
      ...
      if (
        ...
        (target.name === 'transaction' || target.name.includes('transaction'))
      ) {
    ...
    return {
      ...
      CallExpression(node) {
        ...
        switch (property.name) {
          case 'get':
            context.report({ node, messageId: 'noDirectGet' });
            break;
          case 'set':
            context.report({ node, messageId: 'noDirectSet' });
            break;
          case 'update':
            context.report({ node, messageId: 'noDirectUpdate' });
            break;
          case 'delete':
            context.report({ node, messageId: 'noDirectDelete' });
            break;
        }
      },
    };
