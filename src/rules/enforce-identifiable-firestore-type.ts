...
export const enforceIdentifiableFirestoreType = createRule<[], MessageIds>({
  ...
  create(context) {
    ...
    const folderName = path.posix.basename(path.posix.dirname(normalizedFilename));
    ...
    let targetTypeAnnotation: TSESTree.TypeNode | null = null;
    ...
    const checkIdField = (type: TSESTree.Node | null | undefined): boolean => {
      if (!type) return false;
    ...
    const checkIdentifiableExtension = (
      type: TSESTree.Node | null | undefined,
    ): boolean => {
      if (!type) return false;
