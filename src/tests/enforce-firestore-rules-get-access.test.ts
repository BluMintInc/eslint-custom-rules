...
ruleTesterTs.run('enforce-firestore-rules-get-access', enforceFirestoreRulesGetAccess, {
	valid: [
		// Basic good patterns
	...
	invalid: [
		// Basic: direct nested access
		{
			...
			output: "const rules = \"allow read: if resource.data.get('fieldX', null).get('fieldY', null) != null;\";",
		},
		// request.resource variant
		{
			code: "const rules = \"allow update: if request.resource.data.fieldX.fieldY != null;\";",
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow update: if request.resource.data.get('fieldX', null).get('fieldY', null) != null;\";",
		},
		// Single-level property
		{
			code: "const rules = \"allow read: if resource.data.fieldX == null;\";",
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow read: if resource.data.get('fieldX', null) == null;\";",
		},
		// Using === and !== operators
		{
			code: "const rules = \"allow read: if resource.data.foo.bar === null;\";",
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow read: if resource.data.get('foo', null).get('bar', null) === null;\";",
		},
		{
			code: "const rules = \"allow read: if resource.data.foo.bar !== undefined;\";",
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow read: if resource.data.get('foo', null).get('bar', null) !== undefined;\";",
		},
		// Parentheses around expression
		{
			code: "const rules = \"allow read: if (resource.data.user.name != null);\";",
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow read: if (resource.data.get('user', null).get('name', null) != null);\";",
		},
		// Extra whitespace
		{
			code: "const rules = \"allow read: if resource.data.user.name    !=    null;\";",
		...
		// Multiple occurrences in one string (we apply all fixes at once but still report once)
		{
			...
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow read: if resource.data.get('a', null).get('b', null) != null && request.resource.data.get('x', null).get('y', null) == null;\";",
		},
		...
		// Nested gets where inner is missing default
		{
			...
			errors: [{ messageId: 'requireGetDefault' }],
			output: "const rules = \"allow update: if request.resource.data.get('user', null).get('name', null) != null;\";",
		},
		// String with both direct access and missing default in get()
		{
			...
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow read: if resource.data.get('user', null).get('name', null) != null && resource.data.get('email', null) != null;\";",
		},
		// Ensure digits and underscores in field names are handled
		{
			code: "const rules = \"allow read: if resource.data.user_1.name2 != null;\";",
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow read: if resource.data.get('user_1', null).get('name2', null) != null;\";",
		},
		// Undefined comparison variants
		{
			code: "const rules = \"allow read: if request.resource.data.profile.image != undefined;\";",
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow read: if request.resource.data.get('profile', null).get('image', null) != undefined;\";",
		},
		// Multiple problems resolved in one fix
		{
			...
			errors: [{ messageId: 'useGetAccess' }],
			output: "const rules = \"allow read: if request.resource.data.get('a', null).get('b', null) != null || resource.data.get('x', null) == null;\";",
		},
	],
});