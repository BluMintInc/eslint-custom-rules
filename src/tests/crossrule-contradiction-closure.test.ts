...
const KNOWN_DIVERGENT: Record<string, Exemption> = {
  ...
  'enforce-boolean-naming-prefixes::prefer-nullish-coalescing-boolean-props': {
    reason:
      "PIPELINE: 13 blessed texts default a NON-boolean with `||` as scenery for the naming question under test — an object (`assertSafe(config) || { a: 1 }`, `getUser() || {}`), an array (`responseData?.items || []`, three `useX() || []` hook results), a string (`userProfile?.name || 'Anonymous'`, `getMessage() || ''`), a number (`getCount() || 0`), `undefined` (two `findItem(...) || undefined` useState fixtures) — plus two whose operands are unresolved identifiers and calls (`isStale || isOutdated`, `isNotEmpty(value) || isTrimmed(value)`), which the sibling reads as possibly-nullish. The composed `--fix` rewrites every one to `??` and converges with ZERO residual messages under both rules (measured, all 13). Surviving that converged spelling is exactly what #2308 fixed: the owner's fallback shape read only `||`, so the sibling's own fixer turned a report on a defaulted boolean into permanent silence. Reading `??` through the same operand analysis is what keeps the converged text governed — `assertSafe(config) ?? { a: 1 }` still reaches the assert-prefixed carve-out and is silent because the OBJECT fallback settles it, and `isNotEmpty(value) ?? isTrimmed(value)` stays silent through the use-site contradiction carve-out, which reads the `??` spelling in lockstep (measured; with only the detection site widened it reports). The pair joins this list because that fix names `prefer-nullish-coalescing-boolean-props` in this rule's source. The disagreement itself predates the mention: the reporter is the sibling, whose behaviour the fix does not touch, and all 13 texts occur verbatim in the suite before it.",
    cases: { preferNullishCoalescing: 13 },
  },
  'prefer-nullish-coalescing-boolean-props::enforce-boolean-naming-prefixes': {
    reason:
      "INCIDENTAL: the owner's fixtures pin which operand TYPES keep `||` and which convert to `??`, and the sibling objects only to the names carrying those types — the annotated boolean parameters `a` and `b` (five fixtures, including the `T extends boolean` generic arms and one fixed `?? ` output), the annotated arrow `eitherEqual`, and the boolean-literal binding in `const result = false || true;`. All 7 have a spelling clean under BOTH rules: prefixing them (`isFirst`/`isSecond`, `isEitherEqual`, `isResult`) leaves zero messages while the operand types each case exists to pin survive the rename untouched (measured, all 7). The composed `--fix` rewrites NONE of them — the sibling is report-only and the owner has nothing to fix on text it blesses — so the residual is a single-rule NO-OP rather than a standoff (measured, all 7). The pair joins this list because the #2308 fix names `prefer-nullish-coalescing-boolean-props` in the sibling's source; the disagreement itself predates the mention, since the pre-fix sibling reports on exactly the same 7 fixtures (measured).",
    cases: { missingBooleanPrefix: 7 },
  },
...
> = {
  ...
  'enforce-microdiff::enforce-dynamic-imports': {
    reason:
      "The `lodash` import the microdiff fixer deliberately declines to rewrite (`_.difference` is not a deep diff) draws BOTH complaints — the static import and the lodash call are the same remedy. Dropping lodash for `@blumintinc/microdiff`, which is on `DEFAULT_IGNORED_LIBRARIES`, is clean under both (measured). The `enforceMicrodiffImport` residual is the same shape one step earlier: `export const chosen = detailedDiff` holds a value reference of the competing import, which has no call site to rewrite, so the import survives the fix and both rules keep asking for it to go. Binding `chosen` to the `diff` from microdiff instead is clean under both (measured), and is the remedy the report names. The third residual comes from #2322, and is a standoff by construction: a file mixing a convertible competing library with jsdiff or `fast-diff` comes out of the fix with the convertible import retired and the withheld one still standing, so the sibling objects to that surviving static import while `enforceMicrodiffManual` asks for the hand conversion. No fixer may silence it, because rewriting the withheld call is exactly the defect #2322 removed — microdiff answers a different question than a sequence or string diff does. The developer has two ways out, both measured clean under BOTH rules on both fixtures: convert the withheld call to microdiff as well, which drops the static import, or keep the library behind `await import('diff')` / `await import('fast-diff')`, which preserves the algorithm the call needs and satisfies the sibling.",
    residuals: {
      ...
      'enforce-dynamic-imports:dynamicImportRequired + enforce-microdiff:enforceMicrodiffImport + enforce-microdiff:enforceMicrodiffManual': 2,
    },
  },
  'enforce-querykey-ts::prefer-global-router-state-key': {
    reason:
