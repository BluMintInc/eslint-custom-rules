...
ruleTesterTs.run(
  ...
  preferBlockCommentsForDeclarations,
  {
    ...
    function multiLineCommentFunction() {
      ...
      // Bug report case - multiple ESLint directive comments with specific rule names
      `/* eslint-disable */
    /* eslint-disable */
    /* eslint-disable */
    /* eslint-disable */
    ...
    import { ComponentProps, ComponentType, FC, useMemo } from 'react';`,
      ...
      // ESLint directive with scoped plugin rules (block comments)
      `/* eslint-disable */
    const unusedVar = 'test';`,

      // ESLint directive with scoped plugin rules (line comments)
      `// eslint-disable
    const unusedVar = 'test';`,

      // ESLint directive with multiple scoped rules
      `/* eslint-disable */
    const anyVar = 'test';`,

      // ESLint directive with React plugin rules
      `/* eslint-disable */
    const Component = () => ({ test: true });`,

      // ESLint directive with React Hooks plugin rules
      `/* eslint-disable */
    const useCustomHook = () => { return 'test'; };`,

      // ESLint directive with mixed plugin and core rules
      `/* eslint-disable */
    const mixedRules = 'test';`,

      // ESLint directive with complex rule names containing numbers and special chars
      `/* eslint-disable */
    const complexRuleNames = (a: number, b: number, c: number, d: number, e: number) => a + b + c + d + e + 42;`,

      // ESLint directive with plugin prefix variations
      `/* eslint-disable */
    const pluginPrefixVariation = {};`,

      // ESLint enable directive with specific rule names
      `/* eslint-enable */
    const enabledRules = {};`,

      // ESLint env directive with multiple environments
      `/* eslint-env node, browser, es6, jest */
    const multipleEnvs = {};`,

      // Global directive with multiple globals
      `/* global window, document, console, process */
    const multipleGlobals = {};`,

      // Globals directive with multiple globals
      `/* globals window, document, console, process */
    const multipleGlobalsAlt = {};`,

      // ESLint directive with extra spaces and formatting
      `/*   eslint-disable   */
    const extraSpacesFormatting = {};`,

      // Line comment ESLint directive with extra spaces
      `//   eslint-disable
    const lineCommentExtraSpaces = {};`,

      // ESLint directive in JSDoc format should still be ignored
      `/** eslint-disable */
    const jsDocEslintDirective = {};`,

      // Multiple consecutive ESLint directives with different types
      `/* eslint-disable */
    /* eslint-env browser */
    /* global window */
    /* eslint-enable */
    const multipleDirectiveTypes = {};`,
    ],
    invalid: [
      ...
      // Regular block comment that looks like ESLint directive but isn't
      {
        code: `/* This comment mentions eslint-disable but is not a directive */
      const regularComment = 'test';`,
        errors: [{ messageId: 'preferBlockComment' }],
        output: `/** This comment mentions eslint-disable but is not a directive */
      const regularComment = 'test';`,
      },

      // Regular block comment with similar pattern to ESLint directive
      {
        code: `/* disable something else */
      const notEslintDirective = 'test';`,
        errors: [{ messageId: 'preferBlockComment' }],
        output: `/** disable something else */
      const notEslintDirective = 'test';`,
      },

      // Regular block comment with 'global' but not ESLint directive
      {
        code: `/* global-like variable declaration */
      const notGlobalDirective = 'test';`,
        errors: [{ messageId: 'preferBlockComment' }],
        output: `/** global-like variable declaration */
      const notGlobalDirective = 'test';`,
      },

      // Regular line comment that mentions ESLint but is not a directive
      {
        code: `// This comment talks about eslint-disable functionality
      const commentAboutEslint = 'test';`,
        errors: [{ messageId: 'preferBlockComment' }],
        output: `/** This comment talks about eslint-disable functionality */
      const commentAboutEslint = 'test';`,
      },

      // Mixed case: ESLint directive followed by regular comment (only regular comment should be converted)
      {
        code: `/* eslint-disable no-console */
      /* This is a regular comment */
      const mixedComments = console.log;`,
        errors: [{ messageId: 'preferBlockComment' }],
