...
export const dynamicHttpsErrors: TSESLint.RuleModule<'dynamicHttpsErrors', never[]> =
  createRule({
    ...
    defaultOptions: [],
    create(context) {
      const checkForHttpsError = (node: TSESTree.CallExpression | TSESTree.NewExpression) => {
        const callee = node.callee;
        if (
          isHttpsErrorCall(callee)
        ) {
