...
exports.enforceTimestampNow = (0, createRule_1.createRule)({
    ...
    create(context) {
        ...
        const normalizedFilename = node_path_1.default.normalize(context.getFilename());
        const functionsSrcPattern = new RegExp(`(?:^|[\\\\/])functions[\\\\/]src[\\\\/]`);
        // Only apply this rule to backend code (functions/src/)
        if (!functionsSrcPattern.test(normalizedFilename)) {
            return {};
        }
        // Skip test files
        const basename = node_path_1.default.basename(normalizedFilename);
