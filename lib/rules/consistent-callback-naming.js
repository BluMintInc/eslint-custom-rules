...
module.exports = (0, createRule_1.createRule)({
    ...
    meta: {
        ...
        schema: [],
        messages: {
            callbackPropPrefix: 'Callback prop "{{propName}}" is a function but does not use the "on" prefix. The "on" prefix marks event-style props so readers know they must pass a callback instead of data or a component. Rename it to "{{suggestedName}}" (and update the prop definition) so call sites make the callback contract explicit.',
            callbackFunctionPrefix: 'Callback function "{{functionName}}" uses the generic "handle" prefix, which hides the action being performed and makes call sites read like plumbing instead of behavior. Use a verb phrase such as "{{suggestedName}}" to describe the side effect so handlers are self-documenting and easy to scan.',
        },
    },
    defaultOptions: [],
    create(context) {
        ...
        function getHandleSuggestedName(functionName) {
