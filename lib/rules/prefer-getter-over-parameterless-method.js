...
function computeBodyLineCount(body) {
...
function hasNameCollision(node, newName) {
    ...
    const targetIsStatic = node.static ?? false;
    return classBody.body.some((member) => {
        if (member === node) {
            return false;
        }
        const key = member.key;
        if (!key || member.type === utils_1.AST_NODE_TYPES.StaticBlock) {
            return false;
        }
        if (member.computed) {
            return false;
        }
        const memberIsStatic = member.static ?? false;
        ...
        if (key.type === utils_1.AST_NODE_TYPES.Identifier && key.name === newName) {
            return true;
        }
        return false;
    });
}
function hasOverloadSignatures(node) {
    const classBody = node.parent;
    if (!classBody || classBody.type !== utils_1.AST_NODE_TYPES.ClassBody) {
        return false;
    }
    ...
    return classBody.body.some((member) => {
        ...
        const key = member.key;
        if (!key || key.type !== utils_1.AST_NODE_TYPES.Identifier) {
            return false;
        }
        const memberIsStatic = member.static ?? false;
        if (memberIsStatic !== targetIsStatic) {
        ...
        const hasBody = member.value?.body;
...
exports.preferGetterOverParameterlessMethod = (0, createRule_1.createRule)({
    ...
    meta: {
        type: 'suggestion',
        docs: {
            description: 'Enforce getter syntax for synchronous parameterless methods that return values, improving semantic clarity and avoiding accidental method invocation without parentheses.',
            recommended: 'error',
        },
        fixable: 'code',
        schema: [
    ...
    create(context, [options]) {
        ...
        function ensureGlobalPattern(pattern) {
            const flags = pattern.flags.includes('g')
                ? pattern.flags
                : `${pattern.flags}g`;
            return new RegExp(pattern.source, flags);
        }
        function pushChildNodes(current, stack) {
        ...
        function getEnclosingClassBody(node) {
            let current = node;
            while (current) {
        ...
        function addCallUse(node, propName, callUsedNamesByClass, callUsedNamesInFile) {
        ...
        function addCallUseForMember(member, propName, callUsedNamesByClass, callUsedNamesInFile) {
        ...
        function hasSideEffectTag(node) {
            ...
            const negationPattern = /\b(?:no|without|not|non|none|never|free of|lack|lacks|lacking|avoid|avoids|avoiding)\b/;
            /**
             ...
             */
            const matchesAffirmative = (pattern) => {
        ...
        function analyzeMutations(body) {
            ...
            while (stack.length) {
                ...
                if (current.type === utils_1.AST_NODE_TYPES.UnaryExpression &&
                ...
                if (current.type === utils_1.AST_NODE_TYPES.AssignmentExpression) {
                    if (current.left.type === utils_1.AST_NODE_TYPES.MemberExpression) {
        ...
        function isThisProperty(member, propName) {
            if (member.object.type !== utils_1.AST_NODE_TYPES.ThisExpression &&
                member.object.type !== utils_1.AST_NODE_TYPES.Super) {
                return false;
            }
            if (!member.computed &&
        ...
        function bodyReferencesThisProperty(body, propName) {
            const stack = [...body.body];
            while (stack.length) {
                const current = stack.pop();
                if (isFunctionLikeNode(current)) {
        ...
        function isMemberExpressionNode(value) {
            return value.type === utils_1.AST_NODE_TYPES.MemberExpression;
        }
        function trackMemberCall(member, callUsedNamesByClass, callUsedNamesInFile) {
            if (member.computed ||
                member.property.type !== utils_1.AST_NODE_TYPES.Identifier) {
                return;
            }
            ...
            if (propName === 'call' || propName === 'apply' || propName === 'bind') {
                ...
                if (isMemberExpressionNode(target) &&
                    !target.computed &&
                    target.property.type === utils_1.AST_NODE_TYPES.Identifier &&
                    target.object.type === utils_1.AST_NODE_TYPES.ThisExpression) {
                    addCallUseForMember(member, target.property.name, callUsedNamesByClass, callUsedNamesInFile);
                }
                else if (target.type === utils_1.AST_NODE_TYPES.ThisExpression) {
                    addCallUseForMember(member, propName, callUsedNamesByClass, callUsedNamesInFile);
                }
                else if (isMemberExpressionNode(target) &&
                    !target.computed &&
                    target.property.type === utils_1.AST_NODE_TYPES.Identifier) {
                    addCallUseForMember(target, target.property.name, callUsedNamesByClass, callUsedNamesInFile);
                }
                else if (target.type === utils_1.AST_NODE_TYPES.Identifier) {
                    addCallUseForMember(member, target.name, callUsedNamesByClass, callUsedNamesInFile);
                }
                else {
                    addCallUseForMember(member, propName, callUsedNamesByClass, callUsedNamesInFile);
                }
        ...
        function isFunctionReference(member) {
            ...
            if (parent.type === utils_1.AST_NODE_TYPES.ChainExpression) {
                ...
                if (chainParent &&
                    chainParent.type === utils_1.AST_NODE_TYPES.CallExpression &&
                    chainParent.callee ===
                        parent) {
                    return false;
                }
        ...
        function trackMemberReference(member, callUsedNamesByClass, callUsedNamesInFile) {
            ...
            if (member.object.type === utils_1.AST_NODE_TYPES.ThisExpression) {
                addCallUseForMember(member, member.property.name, callUsedNamesByClass, callUsedNamesInFile);
                return;
            }
            addCallUseForMember(member, member.property.name, callUsedNamesByClass, callUsedNamesInFile);
        }
            ...
            for (const prop of pattern.properties) {
                if (prop.type === utils_1.AST_NODE_TYPES.Property) {
                    ...
                    if (key.type === utils_1.AST_NODE_TYPES.Identifier) {
                        addCallUse(pattern, key.name, callUsedNamesByClass, callUsedNamesInFile);
                    }
                    else if (key.type === utils_1.AST_NODE_TYPES.Literal &&
                        typeof key.value === 'string') {
                        addCallUse(pattern, key.value, callUsedNamesByClass, callUsedNamesInFile);
                    }
                }
            }
        }
        ...
        return {
            CallExpression(node) {
                ...
                else if (callee.type === utils_1.AST_NODE_TYPES.ChainExpression) {
                    const expression = callee.expression;
                    if (expression.type === utils_1.AST_NODE_TYPES.MemberExpression) {
                        trackMemberCall(expression, callUsedNamesByClass, callUsedNamesInFile);
                    }
                }
            },
            ...
            'MethodDefinition, TSAbstractMethodDefinition'(node) {
                ...
                if (node.key.type !== utils_1.AST_NODE_TYPES.Identifier)
                    return;
                if (config.ignoreAsync && node.value.async)
                    return;
            ...
            'Program:exit'() {
                ...
                for (const { node, suggestedName } of candidates) {
                    ...
                    const scopeKey = `${(node.static ?? false)
                        ? 'static'
                        : 'instance'}:${suggestedName}`;
                    const existingCounts = suggestedNameCounts.get(classBody) ?? new Map();
                ...
                for (const { node, sideEffectReason, suggestedName } of candidates) {
                    ...
                    context.report({
                        ...
                        fix: sideEffectReason ||
                            ...
                            ? null
                            : (fixer) => fixer.replaceTextRange([node.key.range[0], rightParen.range[1]], `get ${suggestedName}()`),
                    });
                }
            },
        };
...
exports.default = exports.preferGetterOverParameterlessMethod;
//# sourceMappingURL=prefer-getter-over-parameterless-method.js.map