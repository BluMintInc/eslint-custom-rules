...
exports.enforceFirestoreFacade = (0, createRule_1.createRule)({
    ...
    create(context) {
        ...
        const recordRunTransactionCallbackParams = (node) => {
            const isRunTransaction = (isMemberExpression(node.callee) &&
                isIdentifier(node.callee.property) &&
            ...
            if (callback &&
                ...
                if (transactionParam && transactionParam.type === utils_1.AST_NODE_TYPES.Identifier) {
        ...
        const isFirestoreMethodCall = (node) => {
            ...
            if (isIdentifier(object)) {
                ...
                const looksLikeRefName = name === 'ref' ||
                    name.endsWith('Ref') ||
