    ...
    create(context) {
        ...
        return {
            ...
            ExportNamedDeclaration(node) {
                ...
                const isTypeExport = node.exportKind === 'type' || node.declaration?.type === utils_1.AST_NODE_TYPES.TSTypeAliasDeclaration;
                for (const specifier of node.specifiers ?? []) {
                    const specIsTypeExport = isTypeExport || specifier.exportKind === 'type';
                    if (!specIsTypeExport)
                        continue;
                    const exportedName = specifier.exported.type === utils_1.AST_NODE_TYPES.Identifier
