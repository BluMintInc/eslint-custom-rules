...
// Claude Code Stop hook input format
const mockInput = {
...
describe('performPrReviewCheck', () => {
  ...
  it('returns null when not on a PR review branch', () => {
    ...
    const result = performPrReviewCheck(mockInput);
  ...
  it('returns block result when unresolved human comments exist', () => {
...
1. [ ] john-doe — src/components/Button.tsx:42
   - **URL**: https://github.com/BluMintInc/agora/pull/123#discussion_r456789`;
    ...
    expect(result?.reason).toContain(mockCommentsList);
    expect(result?.reason).toContain('npm run resolve-comments');
  });

  it('returns block result when unresolved bot comments exist', () => {
...
1. [ ] coderabbitai — src/hooks/useData.ts:15`;
    ...
    expect(result?.reason).toContain('decisions.json');
  });
