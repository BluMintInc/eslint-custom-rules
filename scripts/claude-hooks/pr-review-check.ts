...
- Step 5: Resolve and Annotate Excluded/Processed Items: Generate \`decisions.json\` from the annotated chat checklist and run \`npm run apply-comment-decisions -- .claude/tmp/decisions.json\` to react with 👍/👎 and resolve threads. The \`decisions.json\` file must be an array of objects with fields:
...
- Step 3: Update Documentation: If applicable, update related \`.claude/skills/\` documentation guides to incorporate captured insights and reflect current behavior.
