...
describe('performMergeConflictCheck', () => {
  ...
  it('returns block result when conflicts remain', () => {
    mockIsInMergeConflictState.mockReturnValueOnce(true);
    ...
    expect(result?.reason).toContain('# Remaining Conflicted Files');
    expect(result?.reason).toContain('Generated prompt content');
  });
  ...
  it('includes checklist output in block reason', () => {
    mockIsInMergeConflictState.mockReturnValueOnce(true);
...
2. [ ] \`src/b.ts\` (3 conflict hunk(s))`;
    ...
    expect(result?.reason).toContain('3 conflict hunk(s)');
  });
});
